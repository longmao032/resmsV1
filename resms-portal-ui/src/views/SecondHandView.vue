...
import { Search, ArrowRight, MapLocation, PriceTag, House, RefreshLeft, Sort, CollectionTag, Menu, List } from '@element-plus/icons-vue'
...
const filters = {
  prices: ['200万以下', '200-300万', '300-400万', '400-500万', '500-800万', '800万以上'],
  types: ['一室', '二室', '三室', '四室', '五室及以上'],
}
...
const priceRanges: Record<string, { min?: number; max?: number }> = {
  '200万以下': { max: 200_000_000 },
  '200-300万': { min: 200_000_000, max: 300_000_000 },
  '300-400万': { min: 300_000_000, max: 400_000_000 },
  '400-500万': { min: 400_000_000, max: 500_000_000 },
  '500-800万': { min: 500_000_000, max: 800_000_000 },
  '800万以上': { min: 800_000_000 },
}

const typeMap: Record<string, string> = {
  '一室': '1室', '二室': '2室', '三室': '3室', '四室': '4室', '五室及以上': '5室',
}
...
async function fetchHouses() {
  ...
  try {
    ...
    if (selectedPrice.value && priceRanges[selectedPrice.value]) {
      ...
      if (range.min) params.minTotalPriceFen = range.min
      if (range.max) params.maxTotalPriceFen = range.max
    }
...
const sortedHouses = computed(() => {
  ...
  if (sortBy.value === 'price') {
    list.sort((a, b) => {
      const pa = a.priceUnit === 1 ? a.unitPrice : a.price
      const pb = b.priceUnit === 1 ? b.unitPrice : b.price
      return pa - pb
    })
...
function formatPrice(item: HousePageItem): string {
  return item.priceUnit === 1 ? String(item.unitPrice) : String(item.price)
}

function formatPriceUnit(item: HousePageItem): string {
  return item.priceUnit === 1 ? '元/㎡' : '万/套'
}
        ...
        <div class="flex flex-col lg:flex-row justify-between items-end gap-8">
          <div class="flex-1">
            <h1 class="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tighter">
              搜寻您的 <span class="text-primary italic">温馨家园</span>
            </h1>
            <p class="text-slate-500 text-lg max-w-2xl font-medium leading-relaxed">
              汇聚全城优质二手房源，真实在售，专业带看。
              当前共有 <span class="text-slate-900 font-bold border-b-2 border-primary/30">{{ total.toLocaleString() }}</span> 套真实在线房源。
            </p>
          </div>
        ...
        <div class="bg-white p-2 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center pr-2 pl-6">
          <div class="flex gap-4 items-center overflow-x-auto no-scrollbar">
            ...
            <div class="flex p-1 bg-slate-50 rounded-xl">
              <button
                v-for="opt in [{ key: 'default', label: '综合排序' }, { key: 'price', label: '总价最低' }, { key: 'area', label: '面积最大' }]"
                :key="opt.key"
